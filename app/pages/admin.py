...
st.set_page_config(page_title="أكاديمية HandsOnSite", page_icon="👨‍💻", layout="wide")
            ...
            if not df_users.empty:
                if att_search_query:
                    df_filtered_attendance = df_users[
                        ...
                        df_users['اسم_المستخدم'].astype(str).str.contains(att_search_query, case=False, na=False)
                    ]
                else:
                    df_filtered_attendance = df_users

                st.markdown(f"### 👥 قائمة الطلاب الظاهرين ({len(df_filtered_attendance)} طالب):")
                
                if not df_filtered_attendance.empty:
                    for idx, row in df_filtered_attendance.iterrows():
                    ...
                    st.warning("⚠️ عذراً، لا يوجد طالب مطابق لكلمة البحث.")
            else:
...
# 9. إضافة الواجبات والتكاليف (مع ديدلاين زمني دقيق)
    ...
    with st.form("assignment_upload_form", clear_on_submit=True):
        ...
        st.markdown("⏰ **حدد الموعد النهائي للتسليم (Deadline):**")
        col_d1, col_d2 = st.columns(2)
        with col_d1:
            asg_date = st.date_input("تاريخ الانتهاء:")
        with col_d2:
            asg_time = st.time_input("وقت الانتهاء:", value=time(23, 59))
        ...
        if submit_asg:
            if asg_title:
                deadline_combined = datetime.combine(asg_date, asg_time).strftime("%Y-%m-%d %H:%M")
                ...
                asg_records.append([asg_title.strip(), deadline_combined, asg_path_str, datetime.now().strftime("%Y-%m-%d %H:%M")])
                ...
                st.success(f"تم نشر الـ Assignment بنجاح! الموعد النهائي: {deadline_combined}")
